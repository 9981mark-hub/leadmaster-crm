...
[자동 생성 예시] 내용 없음`)},1e3)})}try{const i=new Be(o).getGenerativeModel({model:"gemini-1.5-flash"}),a=await Ot(t);return(await(await i.generateContent([Lt,{inlineData:{data:a,mimeType:t.type||"audio/mp3"}}])).response).text()}catch(s){return console.error("Gemini AI Error:",s),`[오류 발생] AI 분석 중 문제가 발생했습니다.
