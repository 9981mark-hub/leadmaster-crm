...
[자동 생성 예시] 내용 없음`)},1e3)})}try{const i=new Be(o).getGenerativeModel({model:"gemini-2.0-flash-lite-preview-02-05"}),a=await Ot(t);return(await(await i.generateContent([Lt,{inlineData:{data:a,mimeType:t.type||"audio/mp3"}}])).response).text()}catch(s){console.error("Gemini AI Error:",s);let i=s instanceof Error?s.message:"알 수 없는 오류";if(i.includes("404")||i.includes("not found"))try{const r=await(await fetch(`https://generativelanguage.googleapis.com/v1beta/models?key=${o}`)).json();if(r.models){const c=r.models.map(h=>h.name.replace("models/","")).join(", ");i+=`
